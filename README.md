## Welcome to Config Patcher!
### Compiling
[Setup Java](#setup-java)

[Setup Git](#setup-git)

[Setup Config Patcher](#setup-config-patcher)

[Compile Config Patcher](#compile-config-patcher)

[Updating Your Repository](#updating-your-repository)

#### Setup Java
The Java JDK is used to compile Config Patcher.
...
#### Setup Git
Git is used to clone Config Patcher and update your local copy.
...
2. Navigate to a place where you want to download Config Patcher's source (eg `C:\Development\Github\Minecraft\`) by executing `cd [folder location]`. This location is known as `mcdev` from now on.
3. Execute `git clone git@github.com:Lunatrius/Config-Patcher.git`. This will download Config Patcher's source into `mcdev`.
4. Right now, you should have a directory that looks something like:

***
    mcdev
    \-ConfigPatcher
        \-ConfigPatcher's files (should have build.gradle)
***

#### Compile Config Patcher
1. Execute `gradlew setupDevWorkspace`. This sets up Forge and downloads the necessary libraries to build Config Patcher. This might take some time, be patient.
    * You will generally only have to do this once until the Forge version in `build.properties` changes.
2. Execute `gradlew build`. If you did everything right, `BUILD SUCCESSFUL` will be displayed after it finishes. This should be relatively quick.
    * If you see `BUILD FAILED`, check the error output (it should be right around `BUILD FAILED`), fix everything (if possible), and try again.
3. Go to `mcdev\ConfigPatcher\build\libs`.
    * You should see a `.jar` file named `ConfigPatcher-#.#.#-#.#.#.#-universal.jar`.
